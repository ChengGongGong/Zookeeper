...
# Zookeeper的角色
    ...
    领导者(leader)：为客户端提供读和写服务，负责进行投票的发起和决议，更新系统状态；
    
    学习者(learner)：为客户端提供读服务，如果是写服务则转发给leader。包括跟随者(Follower),用于接收客户端请求并向客户端返回结果，在选主过程中参与投票，
 ...
 ## 数据模型(Data model)：
    ...
    每个数据节点在zk中被称为znode,它是zk中数据的最小单元，且每个znode都有一个唯一的路径标识，每个节点的数据大小最大是1M，因为zk主要是用于协调服务的而不是存储业务数据的。
